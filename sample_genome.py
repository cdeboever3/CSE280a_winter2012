...
def format_fasta(header, seq, width=50):
    ...
    while (width*i < len(seq)):
        seq_lines.append(sample_seq[width*i: width*(i+1)])
        i += 1
    seq = '\n'.join(seq_lines)
    return '>%s\n%s\n' % (header,seq)
def main():
    parser = argparse.ArgumentParser(description='Randomly selects a contiguous region from a genome.')
    parser.add_argument('--genome-dir', required=True, help='Directory of *.fa files of a genome.')
    parser.add_argument('--output', required=True, help='Output FASTA file')
    parser.add_argument('--sample-length', default=int(1e8), type=int, help='Length of region to sample (Default: 100 Mb).')
