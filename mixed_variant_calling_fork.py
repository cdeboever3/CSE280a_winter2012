...
def main():
    ...
    # genL = [(0, (1 - germ_mu + germ_mu * aa_prob) * (1 - soma_mu)),                  # ('AA','AA')
    #         (0.25, (1 - germ_mu) * soma_mu),                                         # ('AA','AT')
    #         (0.5, germ_mu * at_prob * (1 - soma_mu)) ,                               # ('AT','AT')
    #         (1, germ_mu * tt_prob * (1 - soma_mu)) ,                                 # ('TT','TT')
    #          ]
    ...
    def compute_likelihood(alt_reads, num_reads, normal_freq, normal_alpha, cancer_freq, cancer_alpha, error_rate):
        ...
        # x += sum([pmf_mod(alt_reads + k, num_reads, alt_freq)*poisson_pmf(k, num_reads*error_rate) for k in range(0,num_reads-alt_reads)])
        # x += sum([pmf_mod(alt_reads - k, num_reads, alt_freq)*poisson_pmf(k, num_reads*error_rate) for k in range(0,alt_reads)])
        ...
        # x = 0
        # x += pmf_mod(alt_reads,num_reads,alt_freq) *(1-error_rate)
        # x += pmf_mod(alt_reads-1,num_reads,alt_freq)*error_rate
        # x += pmf_mod(alt_reads+1,num_reads,alt_freq)*error_rate
    ...
    print >>sys.stderr, 'Calculating likelihoods'
    ...
    for normal_alpha, cancer_alpha in parL:
        ...
        print >>sys.stderr, inN, normal_alpha, cancer_alpha, ll
